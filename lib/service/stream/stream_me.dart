/*
import 'dart:async';

import 'package:chatper_book_quiz_core/model_book.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:english_quiz/const/static/global.dart';
import 'package:english_quiz/service/utils/utils.dart';
import 'package:english_quiz/ui/route/route_splash.dart';
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:shared_preferences/shared_preferences.dart';

import '../../const/model/model_user.dart';
import '../../const/static/global.dart';
import '../../const/value/keys.dart';

class StreamMe {
  static StreamSubscription? streamSubscription;

  static Future<void> listenMe(WidgetRef ref) async {
    debugPrint("listenMe ${Global.uid}");
    streamSubscription = FirebaseFirestore.instance.collection(keyUser).doc(Global.uid).snapshots().listen(
      (event) async {
        debugPrint('listenMe 업데이트');
*/
/*        try {
          Global.userNotifier.value = ModelUser.fromJson(event.data()!);
          if (preStudyCompleteCount == null || preStudyCompleteCount != Global.userNotifier.value!.listStudyComplete.length) {
            updateMyStudyComplete();
          }
        } catch (e, s) {
          Utils.toast(desc: 'This user is not exist');
          final pref = await SharedPreferences.getInstance();
          pref.remove(keyUid);
          Navigator.of(Global.contextSplash!).pushAndRemoveUntil(MaterialPageRoute(builder: (context) => const RouteSplash()), (route) => false);
        }
      },*//*

    );
  }

  static Future<void> updateMyStudyComplete() async {
    final listBookUid = Global.userNotifier.value!.listStudyComplete.map((e) => e.bookUid);
    final listFuture = listBookUid.map((e) => FirebaseFirestore.instance.collection(keyBooks).doc(e).get());
    final listDs = await Future.wait(listFuture);
    final listModelBook = listDs.where((ds) => ds.exists).map((e) => ModelBook.fromJson(e.data()!)).toList();
    Global.vnListBookStudyComplete.value = listModelBook;
  }
}
*/
